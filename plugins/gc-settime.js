...
*_Format: .gctime buka/tutup jam_*
*_Contoh: .gctime buka 1_*

*1 = 1 Jam*
`
      	const sections = [
   {
	title: `𝗢𝗣𝗘𝗡 𝗚𝗥𝗢𝗨𝗣 𝗧𝗜𝗠𝗘`,
	rows: [
...
const listMessage = {
  text: ` `,
  footer: '*Atau Bisa Pilih Opsi Dibawah*',
  title: caption,
...
await conn.sendMessage(m.chat, listMessage, { quoted: fakes, mentions: await conn.parseMention(caption), contextInfo:{ forwardingScore: 99999, isForwarded: true }})
  ...
  await conn.groupSettingUpdate(m.chat, isClose).then(async _=> {
	  m.reply(`Sukses Me${isClose == 'nnouncement' ? 'nutup' : 'mbuka'} grup${args[1] ? `, Grup Akan Dibuka Setelah *${clockString(timeoutset)}*` : ''}`)
  })
  if (args[1]) {
	 setTimeout(async () => {
