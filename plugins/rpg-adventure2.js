...
let handler = async (m, { usedPrefix }) => {
    ...
    if (user.health < 190) return m.reply(`
*Dibutuhkan Setidaknya 190HP ❤️ Untuk Berpetualang*
Beli Potion Untuk Return HP Di: *${usedPrefix}buy potion jumlah*,
Dan Ketik *${usedPrefix}heal jumlah* Untuk Menggunakan Potion
`.trim())
if (user.stamina < 190) return m.reply(`
*Dibutuhkan Setidaknya 190ST ⚡ Untuk Berpetualang*
...
`.trim())
if (user.money < 19999) return m.reply(`
*Dibutuhkan Setidaknya 20K Money💵 Untuk Berpetualang*
...
`.trim())
    ...
    let text = `*Anda Telah Berpetualang Ke Arah Barat Dan Sampai Di ${pickRandom(['Amerika Serikat', 'Kanada', 'Meksiko', 'Brasil', 'Argentina', 'Chile', 'Kolombia', 'Peru','Ekuador','Bolivia'])}`
